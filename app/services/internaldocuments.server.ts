export async function getInternalDocuments(){
  const result = await fetch(`http://administracion.grupo-sosamorales.com:8000/api/internal-documents`);
  return await result.json();
}

export async function getInternalDocumentByID(InternalDocumentID: string){
  const result = await fetch(`http://administracion.grupo-sosamorales.com:8000/api/internal-documents/${InternalDocumentID}`)
  return await result.json();
}
