export async function getInternalDocuments(){
  const result = await fetch(`https://grupo-sosamorales.com/api/internal-documents`);
  return await result.json();
}

export async function getInternalDocumentByID(InternalDocumentID: string){
  const result = await fetch(`https://grupo-sosamorales.com/api/internal-documents/${InternalDocumentID}`)
  return await result.json();
}
